...
Form_Aff_Adr::Form_Aff_Adr(QWidget *parent) : QWidget(parent)
{
...
combocritere->addItem("NO_Adresse");
combocritere->addItem("Ligne_Adresse");
combocritere->addItem("Departement");
combocritere->addItem("Commune");
...
void Form_Aff_Adr::Affichage()
{
    ...
    if(db.open())
    {
        ...
        QString pran;
        if(champ->text()=="")
            pran="select* from adresse";
        else
            pran="select* from adresse where "+combocritere->currentText()+"='"+champ->text()+"'";
        model->setQuery(pran);
        if(model)
