...
Form_Aff_Fonc::Form_Aff_Fonc(QWidget *parent) : QWidget(parent)
{
...
void Form_Aff_Fonc::Affichage()
{
    ...
    if(db.open())
    {
        ...
        QString pran;
        if(champ->text()=="")
            pran="select* from fonction";
        else
            pran="select* from fonction where "+combocritere->currentText()+"='"+champ->text()+"'";
        model->setQuery(pran);
        if(model)
